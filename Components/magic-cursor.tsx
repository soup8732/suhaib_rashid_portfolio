...
const defaultConfig: MagicCursorConfig = {
...
const MagicCursor: React.FC<MagicCursorProps> = ({
  ...
  const particlesCursorRef = useRef<any>(null);
  ...
  const initializeParticles = useCallback(() => {
    ...
    const handleClick = () => {
      if (!clickInteraction || !particlesCursorRef.current) return;

      const randomColor =
        interactionColors[Math.floor(Math.random() * interactionColors.length)];
    ...
    if (clickInteraction) targetElement.addEventListener("click", handleClick);
