...
const CodeHoverCards: React.FC<CodeHoverCardsProps> = ({
  ...
  const getColumnClass = () => {
    ...
    return columnMap[columns] || "grid-cols-1";
  };

  return (
    <div className={cn("w-full flex items-center justify-center px-0 py-4 bg-transparent text-foreground", className)}>
      <div className="container mx-auto">
        <div className={cn("grid", getColumnClass())} style={{ gap: cardGap }}>
          {cards.map((card) => {
            const IconComponent = card.icon || DEFAULT_ICONS[0];
            ...
            return (
              <div
                key={card.id}
                className={cn("group relative w-full", disabled && "pointer-events-none opacity-50", cardClassName)}
              >
                <div
                  ref={(el) => {
                ...
                >
                  <div className="relative z-10 text-foreground">
                    <IconComponent size={iconSize} className="transition-transform duration-200 group-hover:scale-110" />
                  </div>
                ...
                {(card.title || card.description) && (
                  <div className="mt-4 text-center">
                    {card.title && <h3 className="text-lg font-semibold text-foreground">{card.title}</h3>}
